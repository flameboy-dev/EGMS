...
function Navbar({ defaultBg = 'bg-[#ECF39E]' }) {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 w-full px-6 py-1.5 md:px-12 md:py-2 lg:px-16 lg:py-2 transition-all duration-300 ${isScrolled
          ? 'bg-white shadow-md'
          : defaultBg
        }`}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        ...
        >
          <img
            ...
            className="h-[40px] w-[40px] object-contain md:h-[72px] md:w-[72px]"
          />
          <span className="font-fredoka text-[22px] font-extrabold leading-none tracking-tight text-[#000000] md:text-[32px]">
            E.G.M.S
          </span>
        </Link>
      ...
      {isOpen && (
        <div className="mt-3 flex flex-col space-y-3 rounded-2xl bg-[#F6FAEF] p-5 shadow-xl lg:hidden">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
